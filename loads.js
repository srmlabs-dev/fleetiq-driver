...
(function (global) {
  ...
  function setLoadStatus(id, status) {
    ...
    _set.loads(_get.loads().map(x => {
      if (x.id !== id) return x;
      // Cancel → pay = 0, keep miles/gross for history
      if (status === 'cancel') {
        return { ...x, status, driverPay: 0, synced: false };
      }
      // Adj → recalc pay from adjAmount if set
      if (status === 'adj' && x.adjAmount) {
        const newPay = calcDriverPayWith(x.adjAmount, x.loadedMiles, x.totalMiles, {
          payType: _get.driver().payType, cpmRate: _get.driver().cpmRate,
          grossPercent: _get.driver().grossPercent, cpmBase: _get.driver().cpmBase,
        }) + Number(x.detention || 0) + Number(x.layover || 0);
        return { ...x, status, driverPay: newPay, synced: false };
      }
      // Active/success → restore original pay from gross
  ...
  function buildReportHTML() {
    ...
    const wonIds    = new Set(disputedList.filter(d => d.status === 'won').map(d => d.loadId));
    const pendingIds = new Set(disputedList.filter(d => d.status === 'pending').map(d => d.loadId));
    const lostIds   = new Set(disputedList.filter(d => d.status === 'lost').map(d => d.loadId));
    ...
    // KPI — based on paid + active only
    const kpiLoads  = [...paidLoads, ...activeLoads];
    ...
    const kpis = [
      { icon: '📦', label: 'Loads',     value: kpiLoads.length },
      ...(isCPM ? [] : [{ icon: '💰', label: 'Gross', value: _fmt(gross) }]),
    ...
    function makeTableSection(title, loads, headerColor, showGross) {
      ...
      const rows = loads.map((x, i) => {
        ...
        return `<tr style="border-bottom:1px solid #e2e8f0;background:${i % 2 === 0 ? '#fff' : '#f8fafc'}">
          <td style="padding:8px 10px;font-weight:600;font-size:12px">${_escHtml(x.loadId)}<br>
            <span style="font-size:10px;color:#94a3b8">${x.pickup || ''}${x.delivery ? ' → ' + x.delivery : ''}</span>
          </td>
          <td style="padding:8px 10px;font-size:11px">${(x.loadedMiles||0).toLocaleString()}<br><span style="color:#94a3b8;font-size:10px">+${(x.deadMiles||0).toLocaleString()}</span></td>
          ${showGross ? `<td style="padding:8px 10px;font-size:12px;font-weight:600">${_fmt(x.gross)}</td>` : ''}
          <td style="padding:8px 10px;font-size:12px;font-weight:700;color:${headerColor}">${_fmt(x.driverPay)}${addlPay > 0 ? `<br><span style="font-size:10px;color:#10b981">+${_fmt(addlPay)}</span>` : ''}</td>
        </tr>`;
      }).join('');

      return `
  <div style="margin-bottom:20px">
    <div style="font-size:12px;font-weight:700;color:${headerColor};margin-bottom:8px;text-transform:uppercase;letter-spacing:.5px">${title} (${loads.length})</div>
    <div style="border:1px solid #e2e8f0;border-radius:10px;overflow:hidden">
      <table class="report-table" style="width:100%;border-collapse:collapse">
        <thead>
          <tr style="background:${headerColor};color:#fff">
            <th style="padding:8px 10px;text-align:left;font-size:11px;font-weight:700">Load ID</th>
            <th style="padding:8px 10px;text-align:left;font-size:11px;font-weight:700">Miles</th>
            ${showGross ? '<th style="padding:8px 10px;text-align:left;font-size:11px;font-weight:700">Gross</th>' : ''}
            <th style="padding:8px 10px;text-align:left;font-size:11px;font-weight:700">My Pay</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
        <tfoot>
          <tr style="background:#f1f5f9;font-weight:800">
            <td colspan="${showGross ? 2 : 2}" style="padding:8px 10px;font-size:11px">${loads.length} load${loads.length !== 1 ? 's' : ''} · ${tmi2.toLocaleString()} mi</td>
            ${showGross ? `<td style="padding:8px 10px;font-size:12px">${_fmt(tgross)}</td>` : ''}
            <td style="padding:8px 10px;font-size:12px;color:${headerColor}">${_fmt(tpay)}</td>
          </tr>
        </tfoot>
      </table>
    </div>
    ...
    const showGross = !isCPM;
    const tablesHTML =
      makeTableSection('✅ Paid', paidLoads, '#10b981', showGross) +
      makeTableSection('🔵 Active', activeLoads, acc, showGross) +
      (disputedLoads.length ? makeTableSection('⚖️ Disputed', disputedLoads, '#f59e0b', showGross) : '') +
      (cancelLoads.length   ? makeTableSection('❌ Cancelled', cancelLoads,  '#ef4444', showGross) : '');
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @media(max-width:600px){
      body{padding:10px}
      .report-table td,.report-table th{padding:5px 6px!important;font-size:10px!important}
    }
  </style>
</head>
<body>
  <!-- HEADER -->
  <div style="display:flex;align-items:center;justify-content:space-between;padding-bottom:14px;border-bottom:3px solid ${acc};margin-bottom:16px">
    <div style="display:flex;align-items:center;gap:12px">
      ...
      <div>
        <div style="font-size:18px;font-weight:900;color:#0f172a">${_escHtml(company)}</div>
  ...
  <!-- SRM gradient line -->
  <div style="height:3px;background:linear-gradient(90deg,transparent,${acc} 20%,#60a5fa 50%,${acc} 80%,transparent);width:100%;margin-bottom:16px"></div>
  ...
  <!-- KPI PILLS -->
  <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:16px">${kpiHTML}</div>
  ${det > 0 || lay > 0 ? `
  <div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap">
    ${det > 0 ? `<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:6px 12px;font-size:12px"><span style="color:#64748b">Detention: </span><strong style="color:#10b981">${_fmt(det)}</strong></div>` : ''}
    ${lay > 0 ? `<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:6px 12px;font-size:12px"><span style="color:#64748b">Layover: </span><strong style="color:#10b981">${_fmt(lay)}</strong></div>` : ''}
  </div>` : ''}
    ...
    document.getElementById('homeLoads').innerHTML = loads.slice(0, 5).map(x => {
      ...
      return `<div class="item" style="border-left:3px solid ${borderColor}">
        <div class="item-inner">
          ...
          <div style="text-align:right">
            ${_get.driver().payType !== 'gross_percent' ? '' : `<div class="item-title" style="color:${isDisputed ? '#f59e0b' : x.status === 'cancel' ? 'var(--rd)' : 'var(--tx)'}">${_fmt(x.gross)}</div>`}
