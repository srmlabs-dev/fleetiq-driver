...
(function (global) {
  ...
  function calcPreview() {
    ...
    const isCPMMode = _get.driver && _get.driver() && _get.driver().payType !== 'gross_percent';
  ...
  function saveLoad() {
    ...
    if (!gross && !isCPM) return _toast('Gross required', 'err');
    const dead      = parseFloat(document.getElementById('deadMiles').value)   || 0;
    const detention = parseFloat(document.getElementById('detentionPay').value)|| 0;
    const layover   = parseFloat(document.getElementById('layoverPay').value)  || 0;
    const total     = loaded + dead;
    ...
    // CPM drivers may not know gross — store 0 if not provided
    const finalGross = gross || 0;
    const entry = {
      id: editId || ('l_' + Date.now()), loadId, gross: finalGross,
  ...
  function buildReportHTML() {
    ...
    const loadsHTML = rLoads.map((x, i) => {
      ...
      return `<tr style="border-bottom:1px solid #e2e8f0;background:${i % 2 === 0 ? '#fff' : '#f8fafc'}">
        <td style="padding:8px 10px;font-weight:600;font-size:12px">${_escHtml(x.loadId)}<br><span style="font-size:10px;font-weight:400;color:#94a3b8">${x.pickup || ''}${x.delivery ? ' → ' + x.delivery : ''}</span><br><span style="font-size:10px;font-weight:400;color:${statusColor}">${statusLabel}</span></td>
        ...
        <td style="padding:8px 10px;font-size:12px;font-weight:600">${_fmt(x.gross)}</td>
        <td style="padding:8px 10px;font-size:12px;font-weight:700;color:${acc}">${_fmt(x.driverPay)}${addlPay > 0 ? `<br><span style="font-size:10px;color:#10b981">+${_fmt(addlPay)}</span>` : ''}</td>
      </tr>`;
    }).join('');
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @media(max-width:600px){
      body{padding:12px}
      .report-table td,.report-table th{padding:6px 8px!important;font-size:11px!important}
    }
  </style>
</head>
<body>
    ...
    <table class="report-table" style="width:100%;border-collapse:collapse">
      <thead>
        <tr style="background:${acc};color:#fff">
          <th style="padding:8px 10px;text-align:left;font-size:12px;font-weight:700">Load ID</th>
          <th style="padding:8px 10px;text-align:left;font-size:12px;font-weight:700">Miles</th>
          <th style="padding:8px 10px;text-align:left;font-size:12px;font-weight:700">Gross</th>
          <th style="padding:8px 10px;text-align:left;font-size:12px;font-weight:700">My Pay</th>
        </tr>
      </thead>
      <tbody>${loadsHTML}</tbody>
      <tfoot>
        <tr style="background:#f1f5f9;font-weight:800">
          <td colspan="2" style="padding:8px 10px;font-size:12px">TOTAL — ${rLoads.length} load${rLoads.length !== 1 ? 's' : ''} · ${tmi.toLocaleString()} mi</td>
          <td style="padding:8px 10px;font-size:13px">${_fmt(gross)}</td>
          <td style="padding:8px 10px;font-size:13px;color:${acc}">${_fmt(pay)}</td>
        </tr>
      </tfoot>
    </table>
  </div>
