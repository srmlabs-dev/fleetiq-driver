...
(function (global) {
  ...
  function buildReportHTML() {
    ...
    const kpis = [
      { label: 'Loads', value: rLoads.length },
      { label: 'Gross', value: _fmt(gross) },
      { label: 'My Pay', value: _fmt(pay) },
      { label: 'Loaded Mi', value: lmi.toLocaleString() },
      { label: 'Dead Mi',   value: dmi.toLocaleString() },
      { label: '$/Mi',      value: tmi ? '$' + (pay / tmi).toFixed(3) : '—' },
    ];

    const kpiHTML = kpis.map(k =>
      `<div style="background:#f8fafc;border-radius:8px;padding:10px 12px;text-align:center;flex:1;min-width:80px">
        <div style="font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">${k.label}</div>
        <div style="font-size:16px;font-weight:800;color:#0f172a">${k.value}</div>
      </div>`
    ).join('');
    ...
    const loadsHTML = rLoads.map((x, i) => {
      ...
      return `<tr style="border-bottom:1px solid #e2e8f0;background:${i % 2 === 0 ? '#fff' : '#f8fafc'}">
        <td style="padding:8px 10px;font-weight:600;font-size:13px">${_escHtml(x.loadId)}</td>
        ...
        <td style="padding:8px 10px;font-size:11px;color:${statusColor}">${statusLabel}</td>
      </tr>`;
    }).join('');
...
<body>
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;padding-bottom:16px;border-bottom:3px solid ${acc};margin-bottom:20px">
    <div style="display:flex;align-items:center;gap:14px">
      ${logo ? `<img src="${logo}" style="height:52px;width:52px;object-fit:contain;border-radius:8px">` : `<div style="width:52px;height:52px;background:${acc};border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:22px">🚚</div>`}
      <div>
    ...
    <table style="width:100%;border-collapse:collapse">
      <thead>
        <tr style="background:${acc};color:#fff">
          ...
          <th style="padding:10px;text-align:left;font-size:12px;font-weight:700">Status</th>
        </tr>
      </thead>
